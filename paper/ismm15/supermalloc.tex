...
\conferenceinfo{ISMM'15}{, June 13-14, 2015, Portland, OR, USA}
\CopyrightYear{2015}
\crdata{/15/06}
\title{SuperMalloc: A Super Fast Multithreaded \texttt{malloc()} for 64-bit Machines}
\authorinfo{Bradley C. Kuszmaul}{MIT CSAIL}{bradley@mit.edu}
\date{}
\maketitle
\begin{abstract}
\bcknote{First-person singular, but not too much.}
...
\end{abstract}
\category{D.4.2}{Operating Systems}
                {Storage Management}
...
arbitrary contiguous page runs,~\footnote{Previously JEmalloc employed
  ...
  red-black trees in 2006--2007.  More recent versions, which we
...
We used JEmalloc 3.6.0, which is licensed under a variant of the
...
Hoard.  Streamflow \cite{SchneiderAnNi06} is also lock-free.  We (the
authors) cannot handle the complexity of lock-free programming for
anything complicated: we can write reliable lock-free code to push an
object onto a linked list, but popping a list is trickier.
...
term apparently coined by \cite{KornVo85}) using no memory.
...
Early versions of JEmalloc used fullest fit, but abandoned it because
although individual groups of equal-sized regions might benefit from
fullest fit, chunks as a whole did not drain as well because a single
``fullest'' page could keep a chunk mapped indefinitely
\cite{Evans15-personal-communication}.
\bcknote{this is not a concern for supermalloc, since we make the entire chunk the same sized objects, and we never unmap an entire chunk} 
...
\bcknote{Explain how we implement fullest fit}
...
\bcknote{Jevans said: You mention relying on uncommitted memory to e.g. make 2 MiB chunks of 16-byte regions cheap, but you also mention how 2 MiB chunks play well with huge pages.  One of the frustrations of transparent huge pages is that they can subvert this reliance on uncommitted memory, resulting in extra CPU and/or memory overhead.  I got the sense that you've considered this issue, but it wasn't very clear whether you explicitly avoid huge pages for small allocations.}
...
\bcknote{what about the server test I wrote in benchmarks:  rss is much better than hoard.}
...
\bcknote{Jevans says: Regarding madvise(ptr, len, MADV\_FREE), you say,
...
\bcknote{Berger says: The only thing I would have added is that Hoard also uses thread-local allocation. }
...
\bcknote{berger says: Otherwise, looks good. I would also note that we talked and implemented OS hooks in Linux to indicate memory pressure or to indicate that a page was being scheduled for eviction, etc., in our work on Bookmarking Garbage Collection (PLDI) and CRAMM (OSDI).}

\cite{HertzFeBe05, YangBeKa06}
...
JEvans helped me understand the role of alignment, explained the
subtleties of \code{MADV_FREE}, proded me to more carefully explain
how I deal with huge pages, and corrected several misconceptions I had
about how JEmalloc works.

\bcknote{Ack funders, also people who contributed}
