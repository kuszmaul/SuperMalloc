...
spaced: the sizes are $8, 10, 12, 14, 16, 20, 24, \ldots, 224$, and
their sizes take the form $k\cdot2^i$ for $4\leq k \leq 7$ and $1\leq
i \leq 5$.  In other words, a small object size, when written in
binary is a $1$, followed by two arbitrary digits, followed by zeros.
Thus bin~$0$ contains $8$-byte objects, bin~1 contains $10$-byte
objects, and so forth.  To compute the bin number from a small size
can be done with bit hacking in $O(1)$ operations using the code shown
in \figref{size2bin}.
...
rather than the locking instructions.  \figref{overhead} shows these
overheads.  We measured the cost of incrementing a global variable
protected by a global lock, and on our E5-2665, that was $485.5$ns.
One alternative is to use a thread-local variable (which is shown on
the last line of the figure), which costs only $3.1$ns.  Most modern
allocators use a thread-local cache of global values.  But it turns
out that a per-CPU lock and variable costs only $34.1$ns to lock and
increment.  The difference is that the lock and the variable mostly
reside in the right cache, and so the only cost is the lock
instruction, the increment instruction, and the call to
\code{sched_getcpu()} to determine which CPU the thread is running on.
If we factor out the cost of \code{sched_getcpu()}, the per-CPU data
structure costs only $13.3$ns.  Based on these numbers, we concluded
that a per-CPU cache gets most of the advantage of a per-thread cache,
but that we still need a small per-thread cache.  Uncontended locking
is not so bad.

One alternative used by some systems is to hash the thread identifier
instead of using a per-CPU cache.  This approach typically requires
more caches than there are CPU's.  If there are $P$ threads, $P$
processors, and $P$ caches, we still expect quite a bit of contention
on some of the caches.  In fact, we expect only $P/e$ threads to
operate without contention.  Since the cost of contention is so high,
this would reduce the cost of access to
...
$P/20$ threads we would get rid of enough contention to get to within
half of optimal.  To get the contention to $1/20$ requires
approximately $3P$ caches.  The biggest advantage to using only $P$
per-CPU caches instead of $3P$ (or $P^2$) caches indexed by a hash of
the thread index, is that the stuff in the cache takes less space.
All the objects in the caches are likely to be using committed memory,
and we want to waste as little physical memory as possible.
...
$23$ns uncontended locking overhead.  So we store just 8~KiB worth of
objects for each size class in the thread cache.  

The per-CPU cache contains just enough objects in each class to fill
up the hardware cache.  We reasoned that that if the working set of a
thread is bigger than the hardware cache, the thread will be incurring
cache misses anyway. There is no point in working hard to avoid a few
more cache misses.  In contrast, many allocators keep many megabytes
in their per-thread caches.  Because there are many per-thread caches
(thousands of threads in a typical database server, for example) their
footprint is larger.

We allocate and remove items from the thread cache with no locking.
If the thread cache gets too full, we move half the objects in the
thread cache to the CPU cache in $O(1)$ operations.  (Similarly, if
the thread cache empties, we move a half-a-cache worth of objects from
the CPU to the thread cache in $O(1)$ operations.)  If the CPU cache
gets too full, we simply start placing objects into the global data
structure, and if all the caches empty, we simply get objects from the
global data structure.  

The thread cache contains two doubly linked lists, each containing at
most $8192$ bytes worth of objects.  When both lists get full, we move
one of the lists to the CPU cache is $O(1)$ operations.

We still need locks for the CPU cache, since there is no guarantee
that we stay on the same CPU for the duration of a critical section.
The CPU cache employs a lock for each CPU-bin pairing.  Since there
are 45~bins and 32~CPUs on the biggest machine we measured, that's
1,440~locks in use.  We place each lock on its own cache line.  For
each bin and CPU, the CPU cache contains two doubly linked lists, each
containing at most $1$MiB worth of objects.  We considered placing the
...
the HTM hardware, we spin on the lock before starting the transaction.
But that spin could cause the transaction to fail.  And on hardware
without HTM, spinning on the lock could interfere with the updates
being made to the linked lists by causing the cache line to bounce
around.
...
%%  LocalWords:  incrementing prefetch
