...
immediately preceeded by 8 bytes which indicate the size of the object
...
it can immediately coaslesce the object with the next object (if it is
...
of the first multithreaded allocators.  The Cilk
...
blowup.  Hoard organizes small objects into \defn{superblocks}, which
contain objects all the same size size.  (Large objects are dealt with
using heavier-weight mechanisms).  Since the objects within a
superblock are all the same, Hoard does not need boundary tags.

Hoard uses an \defn{allocate-fullest} heuristic.  Hoard sometimes
finds itself in a sutation where an object could be allocated out of
...
Mozilla, as well as with MariaDB in recent Red Hat distriutions.
...
tries to allocate.  Like JEmalloc and Hoard, TBBmalloc uses
superblocks each containing objects of homogeneous size.
...
Like Hoard and JEmalloc, SuperMalloc allocates large chunks of
homogeneous-sized objects for small objects, and uses
operating-system-provided memory mapping for larger objects.
SuperMalloc's chunks are multiples of $2$~MiB, and are
$2$~MiB-aligned, which corresponds to huge pages on x86.  In operating
systems that support huge-page page tables, chunks align properly with
the paging.  For objects that are significantly less than $1$~MiB,
SuperMalloc carves chunks into small pieces.  For larger objects, the
allocation is backed by one or more contiguous chunks.
...
large, and huge objects.  The small objects are as small as 8 bytes,
and increase in size by at most 25\% to limit internal fragmentation.
Small objects are regular spaced: the sizes are $8, 10, 12, 14, 16,
20, 24, \ldots, 224$, and their sizes take the form $k2^i$ for $4\leq
k \leq 7$ and $1\leq i \leq 5$.  Thus bin~$0$ contains $8$-byte
objects, bin~1 contains $10$-byte objects, and so forth.  To compute
the bin number from a small size can be done with bit hacking in
$O(1)$ operations using the code shown in \figref{size2bin}.  
...
For small objects, we make no particular effort is made to avoid false
sharing.  Like JEmalloc, we assume that users who care about false
sharing will ask for an object to be cache-line aligned, using e.g.,
...
Given a
pointer to an object, a chunk-based allocator must be able to
determine which chunk an object belongs to, and what the size of the
objects in that chunk are.  
...
%%  LocalWords:  uncontended superblocks TBB TBBmalloc GPLv
